...
/*******************************************************************************
  TEXTURE_MANAGERCLASSの研究

  ※ゲームフレームワークでInitに入る前からTextureやmodelや音のデータを取得しておくようにするために書いてみました。
  <構造体の中にはLPDIRECT3DTEXTURE9と画像のサイズを記入したものを指す>
  今後の課題：テクスチャーローダーはmap<ファイルパス、構造体＞の形で必要なデータを全てつつんで行う。

  Author : AT13A281 31 Mandai Ryuta
********************************************************************************/
...
LPDIRECT3DTEXTURE9 testTexture = nullptr;
...
bool Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow, int nWindowWidth, int nWindowHeight)
{
	...
	Texture_Manager->load(TEXTURERS::texture);
	
	testTexture = Texture_Manager->getResource(TEXTURERS::texture);
